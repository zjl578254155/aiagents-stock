...
> ⚠️ **适用边界声明（2026-07-31 补）**：本规范仅适用于**旧层 DeepSeek 自动分析功能**（首页分析、旧持仓定时分析、智策/智瞰/主力选股等）。
> **EOD 持仓逻辑评估与日内信号模块不适用本规范**——它们遵循 `CLAUDE.md` / `docs/持仓分析工作流SOP.md` 的「程序侧不内嵌 LLM，决策由 Claude 会话做出」定位，两套规范以此为界。
> 另：本文引用的 `openspec/` 目录及根目录 `portfolio_manager.py` 等路径已过期，仅作历史参考。
